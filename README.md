...
## Kanban gadgets (Burnup, Rework, Cycle Time)

TRI Kanban Burnup, TRI Kanban Rework, and TRI Kanban Cycle Time are Kanban-only siblings of the Scrum reporting gadgets above, scoped to a Capacity-tab **iteration** instead of a Jira sprint. Their Edit screen follows a similar-but-distinct flow to "Configuring a gadget" above:

1. **Space** — only lists spaces with **Capacity Planning enabled and a Kanban board** (`getCapacityEnabledProjects({ boardTypeFilter: 'kanban' })`) — these gadgets read a space's configured iterations, so a Scrum space or a Capacity-off space has nothing for them to read.
2. **Iteration** — defaults to **"Active Iteration (auto)"**, re-resolving whichever iteration is currently Active on that space's Capacity tab. Pick a specific iteration instead to pin the widget to it (e.g. a completed one, for a historical view).
3. **Story Points Field** and **4. Status → Phase Mapping** — same concept and same 7-value mapping as the Scrum gadgets (`IterationSourceFields.jsx`, the Kanban sibling of `SprintSourceFields.jsx`). Kept independent of Capacity's own status-*category* based Kanban math — see "Capacity project tab" below.

There's no Commitment Grace Window step and no "Use dashboard sprint filter" option — TRI Kanban Rework/Cycle Time reuse the exact same `computeReworkData`/`computeCycleTimeData` math as their Scrum counterparts (neither takes a grace window), and TRI Sprint Filter's whole model (pick one Jira Sprint) doesn't apply to an app-invented Kanban iteration.

- **TRI Kanban Burnup** — a cumulative burn-up chart: **Target** (a straight ramp from 0 to the iteration's Committed SP, already computed by that space's Capacity tab), **Development**, **Review**, and **Testing** (SP that has reached that stage or any later stage, cumulative, shown as "added since the iteration's start"). No chart/table choice — it's chart-only.
- **TRI Kanban Rework** and **TRI Kanban Cycle Time** — identical concept, chart/table, and columns as TRI Rework/TRI Cycle Time above, just reading an iteration's issue set instead of a sprint's.

All three share one underlying issue fetch (an iteration's "everything active during this date range" issue set, mirroring the two-query merge from the original `jira_kanban_burnup_extract.py` reference script this port is based on) — so having more than one of them on the same dashboard, pointed at the same iteration, costs one Jira fetch, not three.
...
- Every SP sum across the whole app (Burndown, Scope Change, Rework, Cycle Time, Velocity, Capacity, and the three Kanban gadgets) treats an Epic's own SP field as a fallback, not the source of truth: if the Epic has children (found via `parent = <epicKey>`), the sum of the children's SP is used instead, since teams usually point child stories rather than the Epic itself. If a child issue is also independently pulled into the same fetched batch as its Epic (e.g. both land in the same sprint, or both match a Kanban status/date scan — unusual, since Epics aren't normally sprint/board items), that child's SP is counted twice — this is a known, accepted edge case, not something this app tries to deduplicate across issues.
- TRI Kanban Burnup/Rework/Cycle Time's shared "active during window" fetch widens its candidate issue set using each gadget instance's own Status → Phase Mapping (to decide which statuses are worth a `status IN (...)` discovery query, alongside the touched-during-window query). Two differently-mapped gadget instances pointed at the same iteration, refreshing within the same 5-minute cache window, could in theory see a very slightly different candidate set before the per-call `isActiveDuringWindow` filter (the actual correctness check) is applied — the same accepted, bounded tradeoff class as Kanban Committed's `WAS`/`DURING` window above.
- TRI Kanban Burnup's Target line is only as accurate as that iteration's Committed value on the Capacity tab — it never calculates Committed itself. If "Get SP Count" has never been clicked for an iteration, Target ramps from 0 to 0.
