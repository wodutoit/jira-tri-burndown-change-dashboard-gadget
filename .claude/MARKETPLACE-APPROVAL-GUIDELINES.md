...
- **Doesn't break the host Atlassian app UI.** ✅ All 9 gadgets render entirely inside their own Custom UI iframes; nothing touches Jira's surrounding UI. TRI Sprint Filter's cross-gadget updates use `@forge/bridge`'s in-page `events` API — this is a standard Custom UI bridge capability, not a workaround that reaches outside the iframe sandbox.
...
- **Marketing assets (logo/banner/screenshots).** Logo: ✅ done (`static/gadgets/public/icons/app-icon-*.png`). Screenshots: ✅ done, 14 screenshots in `docs/screenshots/` covering the 4 reporting gadgets (including config screens and Chart/Table/Both display modes), embedded into `USAGE.md`. TRI Sprint Filter (added 2026-07-27), TRI Velocity (added 2026-07-28), and TRI Kanban Burnup/Rework/Cycle Time (added 2026-07-31) have no screenshots yet — worth adding before submission. The three Kanban gadgets also temporarily reuse existing gadgets' icon files as thumbnails (see `PROJECT-CONTEXT.md`) — dedicated icons for these three are a pre-submission task too, not just screenshots. Banner image: still not created as of 2026-07-20 — not required for a free listing, worth adding before submission for a stronger listing page.
