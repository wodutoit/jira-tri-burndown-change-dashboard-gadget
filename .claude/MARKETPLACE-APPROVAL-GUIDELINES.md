...
- **Doesn't degrade host app performance.** ⚠️ Mostly fine, but see the known pagination-cap limitation in `PROJECT-CONTEXT.md` — an unusually large sprint could be slow. TRI Velocity (added 2026-07-28) compounds this: a multi-space config fetches up to 10 closed sprints per space on a cache miss, each a full issue+changelog fetch. Not a current violation (closed-sprint data caches indefinitely, so it's a one-time cost per space), but worth re-checking if this ever gets reported as an issue.
